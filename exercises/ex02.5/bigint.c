...
char *reverse_string(char *s) {
    ...
    char *rev_string = malloc(256);
    strcpy(rev_string, s);
    printf("%s \n",rev_string);
    int j;
    int i;
    for(i = 0; rev_string[i] != '\0'; i++){ //find length of string
      j = i;
    }
    for(int z = 0; z <= i-1; z++){ //reverse sring
      rev_string[z] = s[j];
      j--;
    }
    printf("%s \n",rev_string);
...
char itoc(int i) {
    ...
    //if the input was not an integer the compiler would catch it as a type mismatch
    assert(0<i<10);
    return i + '0';
...
void add_digits(char a, char b, char c, char *total, char *carry) {
    ...
    total_val = sum%10;
    carry_out = (sum - total_val) / 10;
    *total =  itoc(total_val);
    *carry = itoc(carry_out);
